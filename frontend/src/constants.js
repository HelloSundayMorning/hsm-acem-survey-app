// export const = '';
export const SET_INTERVIEWER = "SET_INTERVIEWER";
export const UPDATE_BIO = "UPDATE_BIO";
export const UPDATE_LOCATION = "UPDATE_LOCATION";
export const UPDATE_EVALUATION = "UPDATE_EVALUATION";

export const EMAIL_SENDING = "EMAIL_SENDING";
export const EMAIL_SENT = "EMAIL_SENT";
export const EMAIL_FAILED = "EMAIL_FAILED";

export const HISTORY = "HISTORY";

export const POSTING_SURVEY = "POSTING_SURVEY";
export const SURVEY_POSTED = "SURVEY_POSTED";
export const SURVEY_POST_FAILED = "SURVEY_POST_FAILED";

export const ANSWER = "ANSWER";

export const FEEDBACK_POSTING = "FEEDBACK_POSTING";
export const FEEDBACK_POSTED = "FEEDBACK_POSTED";
export const FEEDBACK_POST_FAILED = "FEEDBACK_POST_FAILED";

export const MailChimpList = "A MAILCHIMP LIST";
export const MailChimpURL = "https://us13.api.mailchimp.com/3.0/lists";
export const MailChimpAuth = "Mailchimp auth";
